...
export function validateStatus(data, { now = Date.now(), allowExpired = false } = {}) {
  ...
  if (data.uptime.state === 'reported' && (typeof data.uptime.value !== 'number' || data.uptime.value < 0 || data.uptime.value > 100 || !nonEmpty(data.uptime.window, 40) || !nonEmpty(data.uptime.source_reference) || !Number.isInteger(data.uptime.observations) || data.uptime.observations < 1 || !Number.isInteger(data.uptime.good_observations) || !Number.isInteger(data.uptime.bad_observations) || data.uptime.good_observations + data.uptime.bad_observations !== data.uptime.observations)) fail('invalid reported uptime');
